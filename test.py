...
def main_window(budget):
    ...
    upload_receipt_button = tk.Button(root, text="Upload Receipt", command=upload_image)
    canvas.create_window(root.winfo_screenwidth() // 2 + 200, root.winfo_screenheight() - 100, window=upload_receipt_button)
...
def extract_receipt_data(file_path):
    try:
        ...
        print("Response:\n", response)  # Debuggin (can be deleted later)

        parsed_text = response.choices[0].message.content.strip().strip('```json').strip('```')
        print("Parsed Text:\n", parsed_text)  # Debuggin (can be deleted later)
...
def upload_image():
    ...
    if file_path:
        ...
        if receipt_data:
            ...
            add_expense_from_receipt(store_name, total_price, date)
        else:
            messagebox.showerror("Error", "Could not extract data from the receipt.")

def add_expense_from_receipt(store_name, total_price, date):
    global funds_remaining

    expense_type = "Miscellaneous" 
    price = float(total_price)
    priority = "Medium"  # Defaults for now
